...
WCode signs Znack requests directly with the locally installed CryptoPro CLI tools. Signature configuration, the selected certificate, normalized certificate metadata, and successful test time are stored independently for each shop. WCode never stores or supplies a PIN.

## Required tools

- `cryptcp` signs authentication challenges, SUZ order bodies, True API documents, and local test payloads.
...
WCode searches `PATH` and common CryptoPro installation directories on Windows, Linux, and macOS. Znack Settings does not expose executable paths, arbitrary signer arguments, API host overrides, or other technical fields.
...
3. Use **Refresh certificates** and select a non-expired certificate from CryptoPro's current-user `My` store.
...
`certmgr` output is not assumed to prove private-key availability. Only a successful real `cryptcp` signing test marks that shop `VERIFIED` and proves that the selected certificate's private key is currently usable. Changing the certificate clears verification. Switching shops reloads the selected certificate and verification state of the new shop.
