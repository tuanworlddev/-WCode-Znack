...
class WbMarketplaceApiRateLimiter {
    private static final Logger LOGGER = LoggerFactory.getLogger(WbMarketplaceApiRateLimiter.class);
    ...
    private WbMarketplaceApiRateLimiter() {
    }
    ...
    static void registerRateLimit(String apiKey, Headers headers) {
        ...
        LOGGER.warn("WB Marketplace API rate limit cooldown set for {} ms", cooldownMs);
    }
    ...
    private static void waitQuietly(long waitMs) throws IOException {
        try {
            ...
            throw new IOException("Interrupted while waiting for WB Marketplace API rate limit", ex);
        }
    }
